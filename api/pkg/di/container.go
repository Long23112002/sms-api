...
func (container *Container) AuthHandlerValidator() (validator *validators.AuthHandlerValidator) {
	container.logger.Debug(fmt.Sprintf("creating %T", validator))
	return validators.NewAuthHandlerValidator(
		container.Logger(),
		container.Tracer(),
	)
}
...
func (container *Container) AuthHandler() (handler *handlers.AuthHandler) {
	container.logger.Debug(fmt.Sprintf("creating %T", handler))
...
// RegisterAuthRoutes registers routes for the /auth prefix
