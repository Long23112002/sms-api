...
export const mutations = {
  ...
  setAuthUser(state: State, payload: AuthUser | null) {
    ...
    if (process.client) {
      const { saveAuthUser } = require('~/plugins/axios')
      saveAuthUser(payload)
    }
  },
  setAuthStateChanged(state: State, payload: boolean) {
    state.authStateChanged = payload
  },
...
export const actions = {
  ...
  async loadBillingUsage(context: ActionContext<State, State>) {
    ...
    context.commit('setBillingUsage', response.data.data)
      ...
      // Set empty billing usage on error
      context.commit('setBillingUsage', null)
    }
  },
  ...
  async loadBillingUsageHistory(context: ActionContext<State, State>) {
    ...
    context.commit('setBillingUsageHistory', response.data.data)
      ...
      console.error('Error loading billing usage history:', error)
      // Set empty array on error
      context.commit('setBillingUsageHistory', [])
    }
  },
  ...
  async onAuthStateChanged(
    ...
    // Check if we have custom auth user in localStorage
    // If yes, don't clear it when Firebase auth is null (we're using custom auth)
    if (process.client) {
      const { getAuthUser, getApiKey } = require('~/plugins/axios')
      const customAuthUser = getAuthUser()
      const customApiKey = getApiKey()
      
      // If we have custom auth, ignore Firebase auth state changes
      if (customAuthUser && customApiKey) {
        // Don't clear localStorage if we have custom auth
        return
      }
    }
